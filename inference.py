...
def generate_caption_reduce_repeat(model, image_path, max_length=50, top_p=0.9, temperature=1.0, repetition_penalty=1.2):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.eval()
    try:
        image = Image.open(image_path).convert("RGB")
    except FileNotFoundError: return "Image not found."
...
if __name__ == '__main__':
    ...
    for i, image in enumerate(images_list):
        ...
        # generated_text = generate_caption_reduce_repeat(
        #     model, 
        #     image,
        #     max_length=50,
        #     top_p=0.8,
